  ...
  const renderContent = (text: string) => {
    return text.split("\n\n").map((para, idx) => {
      ...
      if (trimmed.startsWith("### ")) {
        ...
        return (
          <h3 key={idx} id={id} className="text-xl sm:text-2xl font-bold text-slate-900 mt-8 mb-4 font-heading border-b pb-2">
            {title}
          </h3>
        );
      }
      ...
      if (trimmed.startsWith("- ")) {
        ...
        return (
          <ul key={idx} className="list-disc list-inside space-y-2 my-4 text-slate-600 pl-4 text-sm sm:text-base">
      ...
      return (
        <p key={idx} className="text-slate-600 text-sm sm:text-base leading-relaxed mb-6 font-body">
          {trimmed}
        </p>
      );
    });
    ...
    <>
      ...
      <div className="bg-slate-50 min-h-screen py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Link
            href="/blog"
            className="inline-flex items-center text-sm font-bold text-slate-500 hover:text-primary mb-8 group transition-colors"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <article className="lg:col-span-8 bg-white rounded-3xl border border-slate-150 p-6 sm:p-12 shadow-sm">
              {/* Category & Stats */}
              ...
              {/* Title */}
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-display font-extrabold text-slate-950 mb-6 leading-tight">
                {post.title}
              </h1>
              ...
              {/* Share buttons */}
              <div className="border-t border-slate-100 pt-8 mt-12 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex space-x-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center justify-center h-9 px-4 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 text-xs font-semibold gap-1.5 transition-colors border border-slate-200"
                  >
            ...
            <aside className="lg:col-span-4 space-y-8">
              ...
              {toc.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-150 p-6 shadow-sm">
                  <h3 className="text-slate-950 font-bold text-sm uppercase tracking-wider mb-4 font-heading border-b pb-2">
                    Table of Contents
                  </h3>
                    ...
                    {toc.map((heading) => {
                      ...
                      return (
                        <li key={id}>
                          <a
                            href={`#${id}`}
                            className="text-slate-500 hover:text-primary text-xs sm:text-sm leading-relaxed block transition-colors"
                          >
                            {heading}
                          </a>
                        </li>
